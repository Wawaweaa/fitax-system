...
export class XiaohongshuAdapter extends PlatformAdapter {
   ...
   * 平台名称
   */
  readonly name: string = '小红书';

  /**
   * 平台描述
   */
  readonly description: string = '小红书电商平台';
  ...
  async parseFiles(
    ...
    if (ext === '.csv') {
      ...
      throw new Error(`不支持的文件格式: ${ext}，小红书平台支持CSV和Excel格式`);
    }
  }
  ...
  private mapRowToFactRow(
    ...
    // 小红书字段映射
    // 此处需要根据实际小红书结算单字段进行调整
    // 示例字段映射：
    // - 订单号：订单编号/Order No.
    // - 商品编码：商家编码/Merchant SKU
    // - 数量：数量/Quantity
    // - 应收买家：应收买家/Buyer Payment
    // - 应收平台：应收平台/Platform Payment
    // - 价外收费：附加费用/Additional Fee
    // - 平台佣金：平台佣金/Platform Commission
    // - 分销佣金：分销费用/Distribution Fee
    // - 其他费用：其他费用/Other Fee
    // - 结算金额：结算金额/Settlement Amount

    // 尝试获取各个字段（处理字段名可能的变体）
    const orderId = this.getFieldValue(row, ['订单编号', '订单号', 'Order No.', 'Order Number']);
    const skuCode = this.getFieldValue(row, ['商家编码', '商品编码', 'Merchant SKU', 'SKU Code']);
    const quantity = this.getFieldValue(row, ['数量', 'Quantity', 'Qty']);
    const buyerPayment = this.getFieldValue(row, ['应收买家', 'Buyer Payment', 'Buyer Amount']);
    const platformPayment = this.getFieldValue(row, ['应收平台', 'Platform Payment', 'Platform Amount']);
    const additionalFee = this.getFieldValue(row, ['附加费用', '价外收费', 'Additional Fee', 'Extra Charge']);
    const platformComm = this.getFieldValue(row, ['平台佣金', 'Platform Commission', 'Commission']);
    const distributionFee = this.getFieldValue(row, ['分销费用', '分销佣金', 'Distribution Fee', 'Affiliate Fee']);
    const otherFee = this.getFieldValue(row, ['其他费用', 'Other Fee', 'Misc Fee']);
    const settlement = this.getFieldValue(row, ['结算金额', 'Settlement Amount', 'Net Amount']);
    ...
    const factRow: FactRow = {
      ...
      line_count: null, // 小红书可能没有行数信息
      line_no: null,    // 小红书可能没有行号信息
