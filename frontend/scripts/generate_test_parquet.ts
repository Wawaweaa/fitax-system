/**
 * 生成测试Parquet文件脚本 - 不依赖DuckDB，仅生成文件
 */
...
async function generateFactParquet(rows: FactRow[], jobId: string): Promise<void> {
  ...
  // 关闭写入器
  await writer.close();
  console.log(`事实表Parquet文件已生成: ${filePath}`);
}
...
async function generateAggParquet(rows: AggRow[], jobId: string): Promise<void> {
  ...
  // 关闭写入器
  await writer.close();
  console.log(`聚合表Parquet文件已生成: ${filePath}`);
}
 ...
 * 手动创建有效视图文件
 */
async function createEffectiveViewFiles(jobId: string): Promise<void> {
  ...
  const factViewPath = path.join(
    EFFECTIVE_FACT_DIR,
    `user_id=${USER_ID}/platform=${PLATFORM}/year=${YEAR}/month=${MONTH}/effective.json`
  );

  // 确保目录存在
  await ensureDir(path.dirname(factViewPath));
  ...
  const factView = {
    ...
    year: YEAR,
    month: MONTH,
    datasetId: `dataset-${uuidv4().slice(0, 8)}`,
    rowCount: 10,
    rows: [
      {
        rowKey: `${USER_ID}:${PLATFORM}:${YEAR}:${MONTH}:order1:1`,
        uploadId: `upload-${uuidv4().slice(0, 8)}`,
        rowHash: 'dGVzdC11c2VyLTAwMTp3ZWNoYXRfdmlkZW86MjAyNToxMDpvcmRlcjE6MQ=='
      }
    ]
  };

  // 写入事实表有效视图
  await fs.writeFile(factViewPath, JSON.stringify(factView, null, 2));
  console.log(`事实表有效视图已创建: ${factViewPath}`);
  ...
  const aggView = {
    ...
    year: YEAR,
    month: MONTH,
    datasetId: factView.datasetId,
    rowCount: 3,
    uploadIds: [factView.rows[0].uploadId]
  };

  // 写入聚合表有效视图
  await fs.writeFile(aggViewPath, JSON.stringify(aggView, null, 2));
  console.log(`聚合表有效视图已创建: ${aggViewPath}`);
}
...
async function main() {
  try {
    ...
    // 创建有效视图文件
    console.log('创建有效视图文件...');
    await createEffectiveViewFiles(jobId);
    ...
    console.error('生成测试Parquet文件失败:', err);
    process.exit(1);
  }
}
