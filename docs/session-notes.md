...
# Update — 小红书链路（S1 公式对齐与大样本验证）

- 时间（北京时间）：2025-11-20 13:25:00
- 范围：xiaohongshu 平台 S1 明细计算（A–O 15 列）对齐 Excel 模型；在 8 月/10 月大样本数据上的验证；记录金额精度策略（round2）作为待决策点。

## 当前状态

- 公式与实现：
  - `backend/lib/adapters/xiaohongshu_core.ts` 中的 `transformXiaohongshuToFact` 已按以下规范实现：
    - Header 归一化：对结算/订单行的所有键名做 `trim()`（并压缩多余空白），解决了诸如 `" 商品实付/实退 "` 这类带空格列名导致读不到值的问题；
    - 销售数量 `qty_sold`：采用 `demo-1120-小红书模型_规则样例_销售数量新公式_251120.xlsx` 中「小红书-结算账单」H 列的新公式：
      - 分子：`商品实付/实退 + 商家优惠 + 平台优惠补贴`；
      - 分母：订单表中的单价 `商品总价(元) / SKU件数`；
      - 规则：`ABS(ratio) > 0.15` 时用 Excel 等价的 `ROUNDUP(ratio, 0)`，否则用 `ROUNDDOWN(ratio, 0)`；
      - ROUNDUP/ROUNDDOWN 为自实现的 `excelRoundUp0/excelRoundDown0`，对负数采用“绝对值方向取整（远离/靠近 0）”，避免 `Math.ceil/Math.floor` 在负数上的行为差异。
    - 金额 round：增加 `excelRound2`，在 S1 计算中用于两位小数的四舍五入，尽量模拟 Excel 在 `.5` 边界和负数上的行为；目前用于 `net_received` 等金额字段。

- 验证覆盖：
  - demo-1103（规则样例）：
    - 从「小红书-结算账单」导出的 `expected_fact.csv` 与 core 输出对比为 0 diff；
    - 说明在该样本下，A–O 15 列的实现与既有模型一致（包含负数/退款行）。
  - 8 月 test 文件：
    - 结算：`test-上传-小红书_25年8月_结算_251120.xlsx`；订单：`test-上传-小红书_25年8月_订单_251120.xlsx`；
    - 行级 spot-check 与汇总验证显示：`qty_sold` 与各金额字段（含 `net_received`）均能与 Excel 模型对齐。
  - 10 月 test 文件（大样本 ~8000 行）：
    - 结算：`test-上传-小红书_25年10月_结算_251120.xlsx`；订单：`test-上传-小红书_25年10月_订单_251120.xlsx`；
    - `qty_sold` 汇总：Fitax=4508，与 Excel 汇总 4508 完全一致；
    - 其他金额字段汇总也与 Excel 对齐，仅 `net_received` 总和存在 -0.01 的残差（Fitax=1,156,052.78 vs Excel=1,156,052.79）。

## round2 / 金额精度策略 — 待决策点

- 现象：
  - 在 10 月大样本下，`net_received` 总和存在 -0.01 差异；行级抽查金额基本一致，说明误差来自少量行在 `.005` 边界/运费分摊上的舍入差异累积；
  - 针对订单 P772613811239448642 等样本，在 Excel 模型中：
    - 运费分摊列（“收价外收费”）内部使用了高精度值（如 8.995），而非立即 round 成 2 位；
    - `8.995 + 8.995 = 17.99`，从而保证订单级运费守恒；
  - Fitax 当前实现中，在 S1 计算阶段对 `extra_charge` 和 `net_received` 使用了 `excelRound2` 提前截断为 2 位，导致同一订单内多行运费分摊汇总会出现 ±0.01 的偏差。

- 当前决定：
  - 不立刻改变 `round2/excelRound2` 的使用策略，继续保留“行级金额以 2 位小数存储”的实现，作为本阶段小红书链路验收的版本；
  - 将“金额精度策略”记录为 **待决策/可选优化项**：
    - 选项 A：保持现状（存 2 位小数），接受极大样本下 `Σ net_received` 在 0.01 级别的误差；
    - 选项 B：在 core 层移除针对 `extra_charge/net_received` 的 round2，改为：
      - 在内部使用 3–4 位甚至更高精度做中间计算和守恒（特别是运费分摊）；
      - 仅在导出/展示层做格式化到 2 位小数；
    - 选项 C：在订单粒度上增加“最后一行调余额”的精细分摊逻辑，保证单订单内 `Σ extra_charge == 原始运费`，`Σ net_received` 与 Excel 完全一致。

- 后续操作提示：
  - 若未来决定切换到选项 B/C，只需在 `xiaohongshu_core.ts` 中调整 `extra_charge/net_received` 周边的 round2 使用方式和/或分摊算法；
  - 回归验证可直接复用现有的金样对比脚本与 8 月/10 月样本：
    - demo-1103 expected_fact；
    - 8 月/10 月 test 文件的行级 & 汇总对比。

## 已知前端体验问题（大样本预览）

- 在小红书 10 月大样本（~8000 行）下：
  - Worker 已正常完成作业，Parquet 与有效视图正确生成，dataset metadata 中 `factCount` 与 aggCount 正常；
  - Upload 页“处理并预览”逻辑会在 `process` 返回后轮询 `/api/preview?view=fact`，若在超时时间内一直接收到空数据/错误，则提示：
    - “处理完成但预览未就绪，请稍后重试或刷新”；
  - 根据 dev 日志，Worker/预览在大数据量下偶尔会在轮询窗口之后才变为可用，或在某次 preview 请求中返回 500/空数组，导致前端误判为“未就绪”。

- 后续需要单独梳理的大样本体验：
  - 是否降低初始预览的 `pageSize`（当前默认为 10000 行），用分页或懒加载改善响应时间；
  - 是否放宽轮询超时阈值，或在前端根据 job 状态（`/api/jobs/:jobId`）来判断“处理完成”，而不是单纯依赖 preview 返回非空；
  - 针对大样本场景，可以在 UI 上区分：
    - 处理失败（job.status=failed）；
    - 处理成功但预览查询异常（提示用户稍后点击“重试”或仅使用导出功能）。
